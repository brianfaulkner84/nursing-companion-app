...
export async function POST(request: Request) {
  ...
  // school_admin sits between instructor and admin -- scaffolded in beta_codes.role's check
  // constraint (see migration 20260807100000_add_school_admin_role.sql) even though nothing
  // generates a school_admin code yet, so this stays correct the day one exists instead of
  // needing a matching code change at that point.
  const rolePrecedence: Record<string, number> = { student: 0, instructor: 1, school_admin: 2, admin: 3 };
