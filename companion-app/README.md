...
Three active roles: `student`, `instructor`, `admin`, stored on `profiles.role`. An instructor
can do everything admin can except manage content: they run `/admin/inbox` (reply to raised
...
A fourth role, `school_admin`, is scaffolded into both `profiles.role` and `beta_codes.role`
(migration `20260807100000_add_school_admin_role.sql`) but nothing grants or checks it yet --
it's a valid value in the database, not a built feature. The intent, for whenever it's built:
an instructor with elevated permissions scoped to their own school only (remove someone from
the school, downgrade a role, manage that school's codes), as opposed to `admin` which is
global across every school. `canReviewStudents` in `lib/roles.ts` already treats it as at
least instructor-level, so a school_admin isn't accidentally locked out of anything an
instructor can already do; the actual elevated capabilities need their own routes and their
own helper alongside `canReviewStudents`/`isAdmin`, scoped by `school_id` the same way an
instructor's queues already are.
...
A `/report-card` page (`lib/report-card.ts` does the actual rollup, shared so an
instructor-facing version can reuse it later) gives a student their overall
completion and accuracy, an NCLEX-category breakdown, and a focus-areas/strengths
list built from subjects with at least `MIN_SAMPLE_SIZE` (5) answered questions --
below that, a subject is left out of both lists rather than shown with a number that
doesn't mean anything yet. Multiple attempts on the same question use the most
recent one, not the first, so a subject a student struggled with early and has since
gotten right doesn't stay flagged. "Export to PDF" is `window.print()` with a print
stylesheet that hides the site header and on-page buttons, not a PDF library --
that's a deliberate choice to keep the dependency list small, the browser already
does this well via its Save as PDF print destination.
